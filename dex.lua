local function deserialize(bytecode)
    ...
    do
        ...
        function reader:nextInt()
            ...
            return (bit32.bor(
                bit32.lshift(b[4], 24),
                bit32.bor(bit32.lshift(b[3], 16), bit32.bor(bit32.lshift(b[2], 8), b[1]))
            ))
        end
...
luau.ISK = function(x)
    return bit32.band(x, luau.BITRK)
end
luau.INDEXK = function(x)
    return bit32.band(x, bit32.bnot(luau.BITRK))
end
luau.RKASK = function(x)
    return bit32.bor(x, luau.BITRK)
end
luau.MASK1 = function(n, p)
    return bit32.lshift(bit32.bnot(bit32.lshift(bit32.bnot(0), n)), p)
end
luau.MASK0 = function(n, p)
    return bit32.bnot(luau.MASK1(n, p))
end
luau.GETARG_A = function(i)
    return bit32.band(bit32.rshift(i, luau.POS_A), luau.MASK1(luau.SIZE_A, 0))
end
luau.GETARG_B = function(i)
    return bit32.band(bit32.rshift(i, luau.POS_B), luau.MASK1(luau.SIZE_B, 0))
end
luau.GETARG_C = function(i)
    return bit32.band(bit32.rshift(i, luau.POS_C), luau.MASK1(luau.SIZE_C, 0))
end
luau.GETARG_Bx = function(i)
    return bit32.band(bit32.rshift(i, luau.POS_Bx), luau.MASK1(luau.SIZE_Bx, 0))
end
...
luau.GETARG_sAx = function(i)
    return bit32.rshift(i, 8)
end
luau.GET_OPCODE = function(i)
    return bit32.band(bit32.rshift(i, luau.POS_OP), luau.MASK1(luau.SIZE_OP, 0))
end
...
local function disassemble(a1, showOps)
    ...
    if (typeof(a1) == "Instance") then
        if not getscriptbytecode then
            error("Executor does not support getscriptbytecode")
        end
            ...
            if markedAux then
                ...
                elseif opc == getOpCode("DUPTABLE") then
                    ...
                    for j = 1, t.size do
                        ...
                        output =
                            output .. ((type(k.value) == "string") and ('"' .. k.value .. '"') or tostring(k.value))
                        if j < t.size then
                            output = output .. ", "
                        end
                    end
...
local dumpbytecode = disassemble
if type(dumpbytecode) == "function" then print("bytecode dumper success") end
local nodes = {}
local selection
...
local function main()
	...
	ScriptViewer.ViewScript = function(scr)
		local source = env.decompile(scr)
